...
-- BEE HUB 🐝🍯 - INSTANT STEAL EGG in SCRIPTS tab
...
-- fireproximityprompt executor function (may be nil in Studio)
local fireproximityprompt = rawget(getfenv(), "fireproximityprompt") or rawget(_G, "fireproximityprompt")
...
local activeTab = tabs[1] -- start on SCRIPTS
...
local function pulseGlow(row, color)
    ...
    pg.AnchorPoint = Vector2.new(0.5, 0.5)
    pg.Position = UDim2.new(0.5, 0, 0.5, 0)
    pg.Size = UDim2.new(0, 20, 0, 20)
    pg.BackgroundColor3 = color
    ...
    TweenService:Create(pg, TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
        Size = UDim2.new(1.4, 0, 3, 0),
        BackgroundTransparency = 1,
    }):Play()

    task.delay(0.55, function() pg:Destroy() end)
end
...
local function createRow(labelText, order, onActivate)
    ...
    activate.Size = UDim2.new(0, 90, 0, 34)
...
local function applyTheme(r, g, b)
    ...
    TweenService:Create(glow, TweenInfo.new(0.3), {ImageColor3 = accent}):Play()
...
-- =========================================================
-- 🥚 INSTANT STEAL EGG (from your code)
-- =========================================================
local stealConnections = {}

local function startInstantStealEgg()
    ...
    table.insert(stealConnections, Workspace.DescendantAdded:Connect(optimizePrompt))

    table.insert(stealConnections, ProximityPromptService.PromptButtonHoldBegan:Connect(function(prompt)
        firePrompt(prompt)
    end))

    table.insert(stealConnections, UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    print("[BEE HUB] INSTANT STEAL EGG enabled. Press [B] to suck nearby eggs.")
end

local function stopInstantStealEgg()
    for _, c in ipairs(stealConnections) do
        pcall(function() c:Disconnect() end)
    end
    stealConnections = {}
    print("[BEE HUB] INSTANT STEAL EGG disabled.")
end
...
local function buildScriptsList()
    createRow("INSTANT STEAL EGG", 1, function(isOn)
        if isOn then
            startInstantStealEgg()
        else
            stopInstantStealEgg()
        end
    end)
...
print("[BEE HUB 🐝🍯] Loaded. SCRIPTS tab = INSTANT STEAL EGG.")