...
local Config = {
    ...
    -- Colores (Tema Amarillo)

    GlobalESPColor = Color3.fromRGB(255, 255, 0),

    CustomESPColor = Color3.fromRGB(255, 220, 0),
...
if not RenderedEggsFolder then
    ...
    warn("[Eggs ESP v2.3] No se encontró Workspace.RenderedEggs. La GUI igual se abrirá.")
...
local function updateEggESP(egg)
    ...
    -- El ESP individual tiene prioridad.
...
local function updateMovementModeButtons()
    ...
    if movementMode == "AutoFarm" then
        ModeAutoFarmBtn.BackgroundColor3 = Color3.fromRGB(180, 150, 0)
        ...
        ModeTeleportBtn.BackgroundColor3 = Color3.fromRGB(200, 170, 0)
...
local function setMovementMode(mode)
    ...
    if StatusLabel then
        StatusLabel.Text = mode == "AutoFarm" and "● Modo AutoFarm: movimiento + noclip (500)" or "● Modo Teleport: TP instantáneo"
...
local function setAutoFarmButtonState(button, active)
    ...
    if active then
        ...
        button.BackgroundColor3 = Color3.fromRGB(180, 150, 0)
...
local function startAutoFarm()
    ...
    autoFarmThread = task.spawn(function()
            ...
            for _, egg in ipairs(eggs) do
                ...
                if isValidEgg(egg) and not autoFarmProcessed[egg] then
                    ...
                    if success and autoFarmActive then
                        ...
                        -- Evita repetir el mismo objeto si el juego todavía no lo elimina.
        ...
        if not autoFarmActive then
            ...
            StatusLabel.Text = "● AutoFarm terminado: no quedan Eggs seleccionados"
...
DeviceFrame.Size = UDim2.new(0, 320, 0, 150)

DeviceFrame.Position = UDim2.new(0.5, -160, 0.5, -75)
...
DeviceStroke.Color = Color3.fromRGB(70, 70, 70)
...
PCBtn.Size = UDim2.new(0, 125, 0, 50)

PCBtn.Position = UDim2.new(0, 25, 0, 70)
...
MobileBtn.Size = UDim2.new(0, 125, 0, 50)

MobileBtn.Position = UDim2.new(1, -150, 0, 70)
...
AuthorLabel.Position = UDim2.new(0, 12, 0, 5)
...
GameLabel.Position = UDim2.new(0, 12, 0, 42)
...
MinimizeBtn.Position = UDim2.new(1, -40, 0, 13)
...
ContentContainer.Size = UDim2.new(1, -24, 1, -72)

ContentContainer.Position = UDim2.new(0, 12, 0, 66)
...
ModeAutoFarmBtn.Position = UDim2.new(0, 0, 0, 26)
ModeAutoFarmBtn.BackgroundColor3 = Color3.fromRGB(180, 150, 0)
ModeAutoFarmBtn.BackgroundTransparency = 0.10
ModeAutoFarmBtn.Text = "Modo AutoFarm"
ModeAutoFarmBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
...
ModeTeleportBtn.Position = UDim2.new(0.5, 4, 0, 26)
ModeTeleportBtn.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
ModeTeleportBtn.BackgroundTransparency = 0.18
ModeTeleportBtn.Text = "Modo Teleport"
ModeTeleportBtn.TextColor3 = Color3.fromRGB(220, 220, 220)
...
StopAutoFarmBtn.Text = "■ Parar AutoFarm"
...
StopAutoFarmBtn.MouseButton1Click:Connect(function()
    ...
    StatusLabel.Text = "● AutoFarm detenido"
...
ToggleGlobalESPBtn.MouseButton1Click:Connect(function()
    ...
    if mainESPActive then
        ...
        ToggleGlobalESPBtn.Text = "ESP Todos: ON"

        ToggleGlobalESPBtn.TextColor3 = Color3.fromRGB(255, 220, 0)

        StatusLabel.Text = "● ESP activo"
        ...
        StatusLabel.Text = "● ESP desactivado"
...
AutoBestEggBtn.Position = UDim2.new(0, 0, 0, 114)

AutoBestEggBtn.BackgroundColor3 = Color3.fromRGB(35, 35, 35)

AutoBestEggBtn.BackgroundTransparency = 0.18

AutoBestEggBtn.Text = "Auto Best Egg: OFF"

AutoBestEggBtn.TextColor3 = Color3.fromRGB(220, 220, 220)

AutoBestEggBtn.TextSize = 14

AutoBestEggBtn.Font = Enum.Font.SourceSansBold
...
AutoBestEggBtn.MouseButton1Click:Connect(function()
    ...
    if autoBestEggActive then
        ...
        AutoBestEggBtn.TextColor3 = Color3.fromRGB(255, 220, 0)

        StatusLabel.Text = "● Auto Best Egg activo"
...
TPHomeBtn.Position = UDim2.new(0, 0, 0, 158)
...
TPHomeBtn.MouseButton1Click:Connect(function()
    ...
    if success then
        ...
        StatusLabel.Text = movementMode == "AutoFarm" and "● Movido a Inicio (500 + noclip)" or "● Teleportado a Inicio"

        StatusLabel.TextColor3 = Color3.fromRGB(255, 220, 0)

    else

        StatusLabel.Text = "● No se encontró tu parcela"
...
KeybindBtn.MouseButton1Click:Connect(function()
    ...
    KeybindBtn.TextColor3 = Color3.fromRGB(255, 230, 0)
...
ListContainerFrame.Position = UDim2.new(0, 0, 0, 282)
...
ListStroke.Color = Color3.fromRGB(55, 55, 55)
...
EggCountLabel.Text = "Eggs detectados: 0"

EggCountLabel.TextColor3 = Color3.fromRGB(170, 170, 170)
...
SortBtn.Size = UDim2.new(0.48, -5, 0, 28)

SortBtn.Position = UDim2.new(0.52, 0, 0, 27)

SortBtn.BackgroundColor3 = Color3.fromRGB(45, 45, 45)

SortBtn.BackgroundTransparency = 0.15

SortBtn.Text = "Orden: Nombre"
...
SearchBox.PlaceholderText = "🔍 Buscar modelo..."
...
ScrollList.Size = UDim2.new(1, -10, 1, -95)
...
local function createEggListItem(egg, itemHeight, textSize)
    ...
    AutoFarmBtn.MouseButton1Click:Connect(function()
        ...
        -- Permite volver a procesar Eggs del mismo nombre.
        ...
        if autoFarmEggs[name] then
            ...
            StatusLabel.Text = "● AutoFarm seleccionado: " .. name
            ...
            StatusLabel.Text = "● AutoFarm quitado: " .. name
...
local function populateList()
    ...
    -- Primero limpiamos la lista completa.

    clearEggList()

    if not RenderedEggsFolder then

        EggCountLabel.Text = "Eggs: 0  |  Resultados: 0"
    ...
    local itemHeight = isMobileMode and 32 or 34
    ...
    -- Solo mostramos el mensaje si realmente no hay resultados.

    -- Ahora se crea una sola vez porque la lista se limpia antes.
    ...
    if visibleCount == 0 then
        ...
        emptyLabel.Text = "No se encontraron Eggs"
...
RefreshBtn.MouseButton1Click:Connect(function()
    ...
    StatusLabel.Text = "● Lista actualizada"
...
ToggleListBtn.MouseButton1Click:Connect(function()
    ...
    if ListContainerFrame.Visible then
        ...
        ToggleListBtn.Text = "Ocultar Lista de Eggs ▲"
    ...
    if listeningForKey then
        ...
        if input.UserInputType == Enum.UserInputType.Keyboard then
            ...
            KeybindBtn.Text =
                ...
                "Tecla TP Inicio: [" .. tpKeybind.Name .. "]"
...
local function setMobileMode()
    ...
    TitleLabel.Position = UDim2.new(0, 10, 0, 17)
    ...
    ContentContainer.Position = UDim2.new(0, 10, 0, 56)
    ...
    ToggleGlobalESPBtn.Position = UDim2.new(0, 0, 0, 60)

    ToggleGlobalESPBtn.TextSize = 12

    AutoBestEggBtn.Size = UDim2.new(1, 0, 0, 34)

    AutoBestEggBtn.Position = UDim2.new(0, 0, 0, 98)

    AutoBestEggBtn.TextSize = 12

    TPHomeBtn.Size = UDim2.new(1, 0, 0, 34)

    TPHomeBtn.Position = UDim2.new(0, 0, 0, 136)

    TPHomeBtn.TextSize = 12

    KeybindBtn.Size = UDim2.new(1, 0, 0, 28)

    KeybindBtn.Position = UDim2.new(0, 0, 0, 174)

    KeybindBtn.TextSize = 11

    ToggleListBtn.Size = UDim2.new(1, 0, 0, 34)

    ToggleListBtn.Position = UDim2.new(0, 0, 0, 206)

    ToggleListBtn.TextSize = 12

    ListContainerFrame.Size = UDim2.new(1, 0, 0, 200)

    ListContainerFrame.Position = UDim2.new(0, 0, 0, 248)
...
print("[Eggs ESP v2.3 - Yellow Theme] Cargado correctamente.")